...
def FindLibrary(library_name = "XStreamDrv", library_loader = _library_loader):
    ...
    library_path = r"C:\Users\me\PycharmProjects\idthse\XStreamDrv.dll"
    if library_path:
...
def LoadLibrary(library_name = "XStreamDrv"):
    ...
    if XStreamDrv.__class__ is DelayedLoadLibrary:
        ...
        raise LibraryAlreadyLoadedException("Dynamic library was loaded on module import and can not be changed")
...
class XS_CAM_MODEL(XSULONG32):
    ...
    XS_CM_XSM_STICK=50              # TB X-Stream Mini Stick
    XS_CM_XSM_4K  = 51              # TB X-Stream Mini 4K
    XS_CM_XSM_8K  = 52              # TB X-Stream Mini 8K
    XS_CM_XSM_4KX2= 53              # TB X-Stream Mini 4K-X2
    XS_CM_R_2K    = 55              # R-series 2K
    XS_CM_CC_STICK= 56              # CrashCam Mini Stick
...
class XS_LINK_TYPE(XSULONG32):
    ...
    XS_LT_10GIGAETH = 8             # 10 Gigabit Ethernet
...
class XS_SYNCIN_CFG(XSULONG32):
    ...
    # IRIG/DTS/STP
    XS_SIC_IRIG_DTS_EXT = 5         # acquisition is synced by IRIG GPS signal with external 1pps generation
    XS_SIC_IRIG_DTS_INT = 6         # acquisition is synced by IRIG GPS signal with internal 1pps generation (N-only)
    XS_SIC_1PPS         = 7         # acquisition is synced by 1PPS signal entering from the sync in connector
...
class XS_SYNCOUT_CFG(XSULONG32):
    ...
    XS_SOC_CFGWID_INV = 3           # the sync out width is configurable and the signal is inverted
...
class XS_CI_MODE(XSULONG32):
    ...
    XS_CIM_ADVANCED2   = 2          # more advanced demosaic
...
class XS_LUT(XSULONG32):
    ...
    XS_LUT_G14    = 7               # Pre-defined LUT: Gamma 1.4
    XS_LUT_G18    = 8               # Pre-defined LUT: Gamma 1.8
    XS_LUT_G20    = 9               # Pre-defined LUT: Gamma 2.0
    XS_LUT_REC709 = 10              # Pre-defined LUT: REC 709
    XS_LUT_BT2020 = 11              # Pre-defined LUT: BT 2020
...
class XS_HDMI_MODE(XSULONG32):
    ...
    XS_HDMI_ON          = 1         # HDMI is on, read frame and live are done to both PC and HDMI output)
    XS_HDMI_TRANSFER    = 2         # HDMI is on, read fram and live are done to HDMI output only (PC output is disabled)
    XS_HDMI_INDEPENDENT = 3         # HDMI is on and independent from PC (new designed cameras only)
...
class XS_CALIB_OPCODE(XSULONG32):
    XS_C_BKG_ALL       = 0          # Calibrate background in all conditions (don't overwrite factory file)
...
class XS_JPEG(XSULONG32):
    ...
    XS_JPEG_RAW = 2                 # enable compressed JPEG (JPEG is downloaded from camera and read from SDK)
...
class XS_BATTERY(XSULONG32):
    ...
    XS_BAT_STATE_DISCHARGING   = 0x00000000 # power supply is not connected, running on battery
    XS_BAT_STATE_CHARGING      = 0x00000100 # power supply is connected, battery is charging
    XS_BAT_STATE_FULLY_CHARGED = 0x00000200 # power supply is connected, battery is fully charged
    XS_BAT_DISABLED            = 0x00008000 # the battery is connected but it's disabled
    XS_BAT_UNKNOWN             = 0xffffffff # no battery installed or problem communicating with battery

# Lens Status
class XS_LENS_INFO(XSULONG32):
    XS_LENS_TYPE               = 0x0000000F # lens type (4 bit) 0:no lens 1:Canon 2:MFT ...
    XS_LENS_ZOOM               = 0x00000010 # lens has configurable zoom (0:Prime)
    XS_LENS_MOTOR_ZOOM         = 0x00000020 # lens has motorized zoom  
...
class XS_PARAM(XSULONG32):
    ...
    XSP_BOARD_TEMP   = 69           # board temperature in hundreds of deg C (Y-redesigned cameras only)
    ...
    XSP_PROP_ABORT   = 87           # Post rec Op: if the value is non 0, the Post-Rec opearation is aborted.
    ...
    XSP_BATTERY_STATUS = 119        # battery status (see XS_BATTERY, read-only, NX cameras only)
    XSP_JPEG_QUAL      = 120        # jpeg quality (1 to 100, dfl 90)
    XSP_SSD_STRM_PER   = 121        # inverse of maximum streaming speed to SSD (O-series)
    XSP_SSD_MAX_FRMS   = 122        # max number of frames that can be stored in SSD without overrun
    XSP_GAMMA_LEVEL    = 123        # Gamma Level
    XSP_GAMMA1         = 124        # Gamma 1
    XSP_GAMMA_LEVEL1   = 125        # Gamma level 1
    XSP_MISS_STEP_CNT  = 126        # Number of mission steps that have been configured (readonly)
    XSP_EXEC_STEP_CNT  = 127        # Number of mission steps that have been executed (readonly)
    ...
    XSP_LENS_FOCUS     = 136        # lens focus position
    XSP_LENS_IRIS      = 137        # lens iris/aperture position
    ...
    XSP_SHOCK_TIME     = 140        # Impact sensor shock maximum duration (in multiples of 744 us)
    XSP_LATEST_FRM_IDX = 141        # index of the latest acquired frame (read-only)
    XSP_LENS_ZOOM      = 142        # lens zoom position
    ...
    XSP_PTP_DELAY      = 152        # PTP power up delay [s]
...
class XS_INFO(XSULONG32):
    ...
    XSI_CCAL_FILE_ON        = 41    # the calibration file with current conditions is installed on the hard disk
    XSI_FCAL_FILE_FLASH     = 42    # the factory calibration file is stored in camera flash memory
    ...
    XSI_NEW_DESIGN          = 46    # The camera hardware is redesigned (with internal image processing)
    ...
    XSI_POST_REC_OP         = 53    # the camera supports post-recording operation (write to SD card)
    ...
    XSI_HW_BOARDS           = 61    # number of boards in hardware configuration (0 if not supported)
    XSI_HW_INFO             = 62    # information about boards of Hw configuration ()
    XSI_TNR_SUPPORT         = 63    # the camera supports TNR
    XSI_PIV_READY           = 64    # the camera is PIV ready (timing signals on the status connector)
    ...
    XSI_CI_THR_SUPPORT      = 79    # the camera supports the Color Interpolation threshold value
    ...
    XSI_LENS_INFO           = 88    # lens information
...
class XS_ENUMITEM(ctypes.Structure):
    _fields_ = [
        ...
        ("bDgrSize",       XSULONG32),       # the camera supports configurable datagram size (jumbo packets)
        ("bNewDesign",     XSULONG32),       # the camera is redesigned (Y with on-board image processing)
        ...
        ("nDDRSizeLo",     XSULONG32),       # DDR size in bytes (lo word)
        ("nDDRSizeHi",     XSULONG32),       # DDR size in bytes (hi word)
        ("anFill",    XSULONG32 * 10)        # fill parameters, for future use
        ]
    def __init__(self):
...
class XS_BROC(ctypes.Structure):
    _fields_ = [
        ("nData", XSULONG32 * 1024)           # array of 256*4 uint
        ]
...
# Write To Disk Configuration
class XS_W2DCFG(ctypes.Structure):
    _fields_ = [
    ("nDDRBufSize", XSULONG32),
    ("nOpt", XSULONG32),
    ("nDrives", XSULONG32),
    ("szVolume1", ctypes.c_char * 64),
    ("szVolume2", ctypes.c_char * 64),
    ("szVolume3", ctypes.c_char * 64),
    ("szVolume4", ctypes.c_char * 64),
    ("szDirectory", ctypes.c_char * 512),
    ]
PXS_W2DCFG = ctypes.POINTER(XS_W2DCFG);
...
# stream to disk callback
XS_StreamingCallback = CALLBACK_TYPE(
    None,            # No return is expected from callback
    ctypes.c_void_p, # User defined variable
    XSULONG32,       # Total number of frames saved
    XSULONG32,        # Amount of available buffer (0 to 1000)
    XSULONG32,        # Error code
)
...
class CameraError(Exception):
    ...
    def __init__(self, code):
        self.code = code
        if code in self.error_code_details.keys():
    ...
    @classmethod
    def Create(cls, code):
        if code in cls.error_code_details.keys():
            return cls.error_code_details[code][0](code)
        else:
            return cls(code)

class XS_SUCCESS(CameraError): pass
class XS_E_GENERIC_ERROR(CameraError): pass
class XS_E_NOT_SUPPORTED(CameraError): pass
class XS_E_INVALID_VALUE(CameraError): pass
class XS_E_INVALID_CFG(CameraError): pass
class XS_E_INVALID_HANDLE(CameraError): pass
class XS_E_INVALID_CAMERA_ID(CameraError): pass
class XS_E_INVALID_ARGUMENTS(CameraError): pass
class XS_E_READONLY(CameraError): pass
class XS_E_CAM_ALREADY_OPEN(CameraError): pass
class XS_E_HARDWARE_FAULT(CameraError): pass
class XS_E_BUSY(CameraError): pass
class XS_E_QUEUE_FULL(CameraError): pass
class XS_E_BUFFER_TOO_SMALL(CameraError): pass
class XS_E_TIMEOUT(CameraError): pass
class XS_E_NOT_RECORDING(CameraError): pass
class XS_E_MALLOC(CameraError): pass
class XS_E_ABORTED(CameraError): pass
class XS_E_NOT_IN_FLASH(CameraError): pass
class XS_E_EXP_LICENSE(CameraError): pass
...
CameraError.error_code_details = {
    ...
    #
    XS_ERROR.XS_E_NOT_SUPPORTED:     (XS_E_NOT_SUPPORTED,     "Function is not supported for this device"),
    ...
    XS_ERROR.XS_E_INVALID_CAMERA_ID: (XS_E_INVALID_CAMERA_ID, "Invalid camera id used in XsOpenCamera"),
    XS_ERROR.XS_E_INVALID_ARGUMENTS: (XS_E_INVALID_ARGUMENTS, "Invalid function arguments"),
    ...
    XS_ERROR.XS_E_QUEUE_FULL:        (XS_E_QUEUE_FULL,        "Cannot queue more items, queue is full"),
    XS_ERROR.XS_E_BUFFER_TOO_SMALL:  (XS_E_BUFFER_TOO_SMALL,  "The buffer is too small for an acquisition"),
    XS_ERROR.XS_E_TIMEOUT:           (XS_E_TIMEOUT,           "Operation time-out"),
    XS_ERROR.XS_E_NOT_RECORDING:     (XS_E_NOT_RECORDING,     "the camera is not in record mode"),
    ...
    XS_ERROR.XS_E_NOT_IN_FLASH:      (XS_E_NOT_IN_FLASH,      "the requested information is not stored in flash memory"),
    XS_ERROR.XS_E_EXP_LICENSE:       (XS_E_EXP_LICENSE,       "the time limited license is expired")
    }
...
def XsGetVersion():
    ...
    return (pVersionMS.contents.value, pVersionLS.contents.value, pIsDemo.contents.value)
...
def XsEnumCameras(nEnumFilter):
    ...
    error = XStreamDrv.XsEnumCameras(c_item_list,
                                     ctypes.byref(c_item_count),
                                     nEnumFilter)
    _xs_error_check(error)
...
def XsPreConfigCamera(nCameraId, nParamKey, *value):
    ...
    elif nParamKey in [XS_PRE_PARAM.XSPP_NET_AD_IP,
                       XS_PRE_PARAM.XSPP_CAM_DFL_GW]:
        nValueLo.value = _ip_param_to_num(value[0])
    elif nParamKey in [XS_PRE_PARAM.XSPP_CAM_CMD_PORT,
    ...
    elif nParamKey in [XS_PRE_PARAM.XSPP_DISABLE_1024,
                       XS_PRE_PARAM.XSPP_REBOOT_FW]:
        nValueLo.value = _ip_param_to_num(value[0])
    else:
...
def XsGetCameraInfo(hCamera, nInfoKey):
    ...
    rettype = PXSULONG32
    if nInfoKey in [XS_INFO.XSI_CAMERA_NAME,XS_INFO.XSI_CAL_PATH,XS_INFO.XSI_CAL_NAME]:
...
#*****************************************************************************
#
def XsQueueCameraSettings(hCamera, cfg, pfnCallback = None, nFlags = XS_CALLBACK_FLAGS(), pUserData = None):
    error = XStreamDrv.XsQueueCameraSettings(hCamera,ctypes.byref(cfg),pfnCallback,nFlags,pUserData)
    _xs_error_check(error)
...
def XsReadCameraSettingsArray(hCamera,nOption,pCfgList,pBrocList,pnCfgCnt):
    error = XStreamDrv.XsReadCameraSettingsArray(hCamera,nOption,pCfgList,pBrocList,pnCfgCnt)
    _xs_error_check(error)
...
def XsGetParameterAttribute(hCamera, cfg, nParamKey, nParamAttr):
    ...
    error = XStreamDrv.XsGetParameterAttribute(hCamera,ctypes.byref(cfg),nParamKey,nParamAttr,ctypes.byref(nAttrValue))
    _xs_error_check(error)
...
def XsGetParameter(hCamera, cfg, nParamKey):
    nValue = XSULONG32()
    error = XStreamDrv.XsGetParameter(hCamera,ctypes.byref(cfg),nParamKey,ctypes.byref(nValue))
    _xs_error_check(error)
    return nValue.value
...
def XsReadUserDataFromFlash(hCamera, nType, nDataIDOrOffset, nSize, pDataBuf):
    ...
    error = XStreamDrv.XsReadUserDataFromFlash(hCamera,nType,nDataIDOrOffset,ctypes.byref(nSizeCopy),pDataBuf)
    _xs_error_check(error)
...
def XsWriteUserDataToFlash(hCamera,nType,nDataIDOrOffset,nSize,pDataBuf):
    error = XStreamDrv.XsWriteUserDataToFlash(hCamera,nType,nDataIDOrOffset,nSize,pDataBuf)
    _xs_error_check(error)
...
#*****************************************************************************
#
def XsMemoryStartGrab(hCamera, nStartAddLo, nStartAddHi, nFrames, nPreTrigFrames, pfnCallback, nFlags, pUserData):
    error = XStreamDrv.XsMemoryStartGrab(hCamera, nStartAddLo, nStartAddHi, nFrames, nPreTrigFrames, pfnCallback, nFlags, pUserData)
    _xs_error_check(error)
...
def XsMemoryPreview(hCamera, frame):
    ...
    error = XStreamDrv.XsMemoryPreview(hCamera,ctypes.byref(frame),ctypes.byref(nFrameIndex))
    _xs_error_check(error)
...
def XsMemoryReadFrame(hCamera, nStartAddLo,nStartAddHi,nFrameIdxOrSize,pDataBuf):
    error = XStreamDrv.XsMemoryReadFrame(hCamera, nStartAddLo,nStartAddHi,nFrameIdxOrSize,pDataBuf)
    _xs_error_check(error)
...
#*****************************************************************************
#
def XsMemoryReadFromDisk(hCamera,nMemDstAddLo,nMemDstAddHi,nDiskSrcAddLo,nDiskSrcAddHi,nStartIdx,nStopIdx,pfnCallback,pUserData):
    error = XStreamDrv.XsMemoryReadFromDisk(hCamera,nMemDstAddLo,nMemDstAddHi,nDiskSrcAddLo,nDiskSrcAddHi,nStartIdx,nStopIdx,pfnCallback,pUserData)
    _xs_error_check(error)
...
#*****************************************************************************
#
def XsMemoryDownloadRawFrame(lpszRawFilePath, nStartAddLo, nStartAddHi, nFrameIdx, nPageIdx, nTotFrames):
    error = XStreamDrv.XsMemoryDownloadRawFrame(lpszRawFilePath, nStartAddLo, nStartAddHi, nFrameIdx, nPageIdx, nTotFrames)
...
def XsMemoryReadTriggerPosition(hCamera):
    ...
    error = XStreamDrv.XsMemoryReadTriggerPosition(hCamera, ctypes.byref(nTriggerPosLo), ctypes.byref(nTriggerPosHi), ctypes.byref(nTriggerIndex), ctypes.byref(nTriggerTime))
    _xs_error_check(error)
...
def XsCalibrateNoiseReduction(hCamera, nOpCode, pfnCallback, pUserData):
    error = XStreamDrv.XsCalibrateNoiseReduction(hCamera, nOpCode, pfnCallback, pUserData)
    _xs_error_check(error)
...
def XsGetCameraStatus(hCamera):
    ...
    nInfo2 = XSULONG32()
    nInfo3 = XSULONG32()
    error = XStreamDrv.XsGetCameraStatus(hCamera,
                                         ...
                                         ctypes.byref(nInfo3))
    _xs_error_check(error)
    return (nIsBusy.value, nStatus.value, nErrCode.value,
...
def XsGetAddressList(hCamera, nStartIdx, nAddressCount):
    ...
    error = XStreamDrv.XsGetAddressList(hCamera,
                                        nStartIdx,
                                        nAddressCount,
                                        pnAddressList)
    _xs_error_check(error)
...
#* Param: hCamera - handle to an open camera
#*        nEnable - on/off flag
#*        pCfg - pointer to XS_W2DCFG structure
#*        pfnCallback - pointer to XS_StreamingCallback
#*        pUserData - pointer to user data (returned in callback)
#*
#* Ret:   None
#*
#* Exceptions:
...
#*****************************************************************************
#
def XsConfigureWriteToDisk(hCamera,nEnable,pCfg,pfnCallback,pUserData):
    error = XStreamDrv.XsConfigureWriteToDisk(hCamera,nEnable,pCfg,pfnCallback,pUserData)
    _xs_error_check(error)
...
#*****************************************************************************
#
def XsVideoPlayback(hCamera,nOption,nStartAddLo,nStartAddHi,nFrames,nStartFrameIdx,nStopFrameIdx):
    error = XStreamDrv.XsVideoPlayback(hCamera,nOption,nStartAddLo,nStartAddHi,nFrames,nStartFrameIdx,nStopFrameIdx)
    _xs_error_check(error)
